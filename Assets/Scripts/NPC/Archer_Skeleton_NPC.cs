...
public class Archer_Skeleton_NPC : MonoBehaviour
{
    ...
    [SerializeField] private float currentAttackCooldown_ReadOnly;
        ...
        switch (currentEnemyStates)
        {
            case EnemyStates.Idle_State:
                ...
                if (touchingDirections.IsGrounded && !damageable.lockVelocity)
                {
                    ...
                    if (detectionZone.dectectedEnemyColliders.Count > 0)
                    {
            ...
            case EnemyStates.Attack_State:
                ...
                if (HasTarget = detectionZone.dectectedEnemyColliders.Count > 0)
                {
                    if (currentAttackCooldown <= 0)
                    {
                        GameObject arrow = Instantiate(projectileObject, attackPoint.position, Quaternion.identity);
                        ...
                        {
                            arrow.transform.eulerAngles = new Vector2(arrow.transform.eulerAngles.x, 180);
                        }
                ...
                currentAttackCooldown_ReadOnly = currentAttackCooldown;
                break;
        }
    }
